...
const seedDB = async () => {
  try {
    ...
    console.log('Landlord and Renter seed accounts created:');
    console.log(`- Landlord: landlord@househunt.com (password123)`);
    console.log(`- Renter: sarah@househunt.com (password123)`);

    // Assign properties to landlord and save
    const propertiesWithLandlord = propertiesData.map(p => ({
      ...p,
      landlord: landlord._id
    }));

    await Property.insertMany(propertiesWithLandlord);
    console.log(`${propertiesWithLandlord.length} properties seeded successfully.`);
